...
class ktexHeaderSetter : public ktex_operation_t {
  ...
  public:
    ...
    virtual void call(KTEX::File &ktex) const { ktex.header = h; }
};
...
class imageResizer : public image_operation_t {
  public:
    virtual void call(Magick::Image &img) const {
        ...
        if (options::verbosity >= 0) {
            std::cout << operation_verb << " image to " << img.columns() << "x"
                      << img.rows() << std::endl;
        }
    }
};
...
static inline void generate_mipmaps(ImageContainer &imgs) {
    ...
    if (options::verbosity >= 1) {
        const size_t mipmap_count =
            BitOp::countBinaryDigits(std::min(width, height));
        std::cout << "Generating " << mipmap_count << " mipmaps..."
                  << std::endl;
    }
    ...
    while (width > 0 || height > 0) {
        img.filterType(options::filter);
        img.resize(Magick::Geometry(std::max(width, size_t(1)),
                                    std::max(height, size_t(1))));
        // img.despeckle();
        imgs.push_back(img);
...
class ktexCompressor : public binary_operation_t<KTEX::File &, Magick::Image> {
  ...
  public:
    ktexCompressor(KTEX::File::Header h, int _v = -1)
        : setheader(h), verbosity(_v) {}
    ...
    void compress(KTEX::File &tex, image_container_t &imgs) const {
...

        {
            ...
            std::for_each(first_secondary_mipmap, imgs.end(),
                          ImOp::cleanNoise());
        }
...
class ktexDecompressor : public operation_t<const KTEX::File &, Magick::Image> {
    ...
    template <typename image_container_t>
    void do_decompress(const KTEX::File &tex, image_container_t &imgs,
                       bool _mult_mipmaps) const {
        const int verbosity0 = options::verbosity;
        options::verbosity = std::min(verbosity0, verbosity);
        ...
        std::for_each(imgs.begin(), imgs.end(),
                      Magick::qualityImage(options::image_quality));

        options::verbosity = verbosity0;
    }

  public:
    ktexDecompressor(int _v = -1, bool _mult_mipmaps = false)
        : verbosity(_v), multiple_mipmaps(_mult_mipmaps) {}
