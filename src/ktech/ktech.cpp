...
const IntegerType KTech::BitOp::Pow2Rounder::Metadata<IntegerType>::max_pow_2 =
    Pow2Rounder::roundDown<IntegerType>(
        std::numeric_limits<IntegerType>::max());

/*
 * Returns whether the output path already includes the extension. If explicitly
 * given, this will always be assumed. If the extension is missing, output_path
 * will end with '.'.
 */
...
static void convert_to_KTEX(const PathContainer &input_paths,
                            const string &output_path,
                            const KTEX::File::Header &h) {
    typedef typename PathContainer::const_iterator pc_iter;
    ...
    if (verbosity >= 0) {
        ...
        size_t count = 1;
        for (pc_iter it = ++input_paths.begin(); it != input_paths.end();
             ++it, ++count) {
            cout << ", `" << *it << "'";
    ...
    KTEX::File tex;
    ImOp::ktexCompressor(h, std::min(options::verbosity, 0))
        .compress(tex, imgs);
    tex.dumpTo(output_path, verbosity);
}

static void convert_from_KTEX(std::istream &in, const string &input_path,
                              const string &output_path) {
    const int verbosity = options::verbosity;
    ...
    if (options::info) {
        ...
        ImOp::ktexDecompressor(std::min(options::verbosity, 0),
                               multiple_mipmaps)
            .decompress(tex, imgs);
        ...
        if (verbosity >= 0) {
            if (multiple_mipmaps) {
                ...
                const std::string out_tail = (fmt_end < output_path.length()
                                                  ? output_path.substr(fmt_end)
                                                  : "");

                char buf[3];

                snprintf(buf, sizeof(buf), fmt_string.c_str(), 0);
                std::cout << "Converting RGBA bitmap and saving into `"
                          << out_head << buf << out_tail << "'";
...
static void synthesize_atlas(const VirtualPath &atlas_path,
                             Container input_paths, KTEX::File::Header h) {
    Atlas A;
    ...
    for (image_iterator_t img_it = imgs.begin(); img_it != imgs.end();
         ++img_it, ++path_it) {
...
template <typename Container>
static void analyze_atlas(const VirtualPath &atlas_path, Container input_paths,
                          const VirtualPath &output_dir) {
    (void)input_paths;
...
static void split(OutputIterator it, const string &str,
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        KTEX::File::Header configured_header = parse_commandline_options(
            argc, argv, input_paths, potential_output_path);

        Maybe<VirtualPath> output_path = Just(potential_output_path);

        if (options::atlas_path != nil) {
            if (potential_output_path.hasExtension() &&
                !potential_output_path.hasExtension("tex") &&
                !potential_output_path.isDirectory()) {
        ...
        for (pathlist_t::iterator it = input_paths.begin();
        ...
        const bool is_tex_input =
            input_paths.empty() || input_paths.front().hasExtension("tex");
        ...
        if (is_tex_input) {
            if (options::atlas_path == nil) {
                std::istream *in =
                    input_paths.front().open_in(std::ifstream::binary);

                if (!KTech::KTEX::File::isKTEXFile(*in)) {
                    throw KToolsError(std::string("Input file '") +
                                      input_paths.front() +
                                      "' has '.tex' extension, but its binary "
                                      "contents do not match a KTEX file.");
                }

                if (input_paths.size() > 1) {
                    throw KToolsError("Multiple input files should only be "
                                      "given on TEX output.");
                }
                ...
                if (!output_path.ref().mkdir()) {
                    throw SysError(std::string("failed to create '") +
                                   output_path.ref() + "' directory: ");
                }
                analyze_atlas(options::atlas_path.ref(), input_paths,
                              output_path.ref());
            }
        } else {
            ...
            if (options::atlas_path == nil) {
                ...
                convert_to_KTEX(input_paths, output_path.ref(),
                                configured_header);
            } else {
                synthesize_atlas(options::atlas_path.ref(), input_paths,
                                 configured_header);
            }
        }
