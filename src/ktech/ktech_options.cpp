...
class HeaderStrOptTranslator
    : public StrOptTranslator<KTEX::HeaderFieldSpec::value_t> {
  public:
...
KTEX::File::Header
KTech::parse_commandline_options(int &argc, char **&argv,
                                 std::list<VirtualPath> &input_paths,
                                 VirtualPath &output_path) {
    ...
    try {
        ...
        MyValueArg<string> atlas_path_opt("", "atlas",
        ...
        MyValueArg<string> compression_opt(
            "c", "compression",
            "Compression type for TEX creation. Defaults to " +
                comp_trans.default_opt + ".",
            false, comp_trans.default_opt, &allowed_comps);
        args.push_back(&compression_opt);
        myOutput.setArgCategory(compression_opt, TO_TEX);

        MyValueArg<int> quality_opt(
            "Q", "quality",
            "Quality used when converting TEX to PNG/JPEG/etc. Higher values "
            "result in less compression (and thus a bigger file size). "
            "Defaults to 100.",
            false, 100, "0-100");
        ...
        MyValueArg<string> filter_opt(
            "f", "filter",
            "Resizing filter used for mipmap generation. Defaults to " +
                filter_trans.default_opt + ".",
            false, filter_trans.default_opt, &allowed_filters);
        ...
        MyValueArg<string> type_opt(
            "t", "type",
            "Target texture type. Defaults to " + type_trans.default_opt + ".",
            false, type_trans.default_opt, &allowed_types);
        args.push_back(&type_opt);
        myOutput.setArgCategory(type_opt, TO_TEX);

        SwitchArg no_premultiply_flag("", "no-premultiply",
        ...
        MyValueArg<size_t> width_opt("", "width",
                                     "Fixed width to be used for the output. "
                                     "Without a height, preserves ratio.",
                                     false, 0, "pixels");
        args.push_back(&width_opt);

        MyValueArg<size_t> height_opt("", "height",
                                      "Fixed height to be used for the output. "
                                      "Without a width, preserves ratio.",
                                      false, 0, "pixels");
        args.push_back(&height_opt);

        SwitchArg pow2_opt(
            "", "pow2",
            "Rounds width and height up to a power of 2. Applied after the "
            "options `width' and `height', if given.");
        args.push_back(&pow2_opt);

        SwitchArg square_opt(
            "", "square",
            "Makes the output texture a square, by setting the width and "
            "height to their maximum values. Applied after the options "
            "`width', `height' and `pow2', if given.");
        args.push_back(&square_opt);

        SwitchArg extend_opt(
            "", "extend",
            "Extends the boundaries of the image instead of resizing. Only "
            "relevant if either of the options `width', `height', `pow2' or "
            "`square' are given. Its primary use is generating save and "
            "selection screen portraits.");
        args.push_back(&extend_opt);

        SwitchArg extendleft_opt(
            "", "extend-left",
            "Causes the `extend' option to place the original image aligned to "
            "the right (filling the space on its left). Implies `extend'.");
        args.push_back(&extendleft_opt);

        /*
        str_trans plat_trans("platform");
        ValuesConstraint<string> allowed_plats(plat_trans.opts);
        MyValueArg<string> platform_opt("p", "platform", "Target platform.
        Defaults to " + plat_trans.default_opt + ".", false,
        plat_trans.default_opt, &allowed_plats); args.push_back(&platform_opt);
        myOutput.setArgCategory(platform_opt, TO_TEX);
        */

        SwitchArg info_flag("i", "info",
                            "Prints information for a given TEX file instead "
                            "of converting it.");
        args.push_back(&info_flag);
        myOutput.setArgCategory(info_flag, FROM_TEX);

        MultiSwitchArg verbosity_flag("v", "verbose",
                                      "Increases output verbosity.");
        args.push_back(&verbosity_flag);

        SwitchArg quiet_flag(
            "q", "quiet",
        ...
        MultiArgumentOption multiinput_opt("INPUT-PATH", "Input path.", true,
                                           "INPUT-PATH");
        cmd.add(multiinput_opt);

        /*
         * This only marks the option visually, since all paths are globbed by
         * multiinput_opt.
         */
        ArgumentOption dummy_output_opt("OUTPUT-DIR", "Output path.", false,
                                        "OUTPUT-DIR");
        dummy_output_opt.setVisuallyRequired(true);
        cmd.add(dummy_output_opt);

        for (list<Arg *>::reverse_iterator it = args.rbegin();
             it != args.rend(); ++it) {
            cmd.add(*it);
        }
        ...
        configured_header.setField("compression",
                                   comp_trans.translate(compression_opt));
        configured_header.setField("texture_type",
                                   type_trans.translate(type_opt));
        // configured_header.setField("platform",
        // plat_trans.translate(platform_opt));
        ...
        */
        ...
        std::copy(all_paths.begin(), all_paths.end(),
