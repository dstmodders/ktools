...
class AtlasSheet {
  ...
  private:
    ...
    void update_geometrical_state() const {
        ...
        size_t max_ymax = 0;
        for (bbox_list_t::const_iterator bbox_it = bboxes.begin();
             bbox_it != bboxes.end(); ++bbox_it) {
    ...
    static inline float_type reflect(float_type v) { return 1 - v; }
    ...
    void dump(pugi::xml_node node, const VirtualPath &basedir,
              int verbosity) const;

    void saveImages(const VirtualPath &output_dir, bool clear_on_done,
                    int verbosity) const;

    void analyze_image(imagelist_t::iterator img_it,
                       bbox_list_t::const_iterator bbox_it) const;

  public:
    AtlasSheet(const Atlas *_p = NULL) : _parent(_p) { clear(); }

    AtlasSheet(const AtlasSheet &s) { copyFrom(s); }
    ...
    bool hasSubPath() const { return subpath != nil; }

    const Compat::UnixPath &getSubPath() const { return subpath.ref(); }

    void setSubPath(const Compat::UnixPath &p) const { subpath = Just(p); }

    iterator begin() { return images.begin(); }

    const_iterator begin() const { return images.begin(); }

    iterator end() { return images.end(); }
...
class Atlas {
  ...
  private:
    ...
    void cleanSheetPaths() const {
        if (dirty_sheetpaths) {
            ...
            unsigned int i = 0;
            for (sheet_const_iterator sheet_it = sheets.begin();
                 sheet_it != sheets.end(); ++sheet_it) {
                if (!sheet_it->hasSubPath()) {
                    if (nsheets == 1) {
                        sheet_it->setSubPath(getTexturePath());
                    } else {
                        sheet_it->setSubPath(
                            getTexturePath().removeExtension() +
                            fmt("-%u.tex", i++));
                    }
                }
            }
  ...
  public:
    ...
    const VirtualPath &getTexturePath() const { return texture_path; }
    ...
    const decompressor_t &getDecompressor() const { return *decompressor; }

    void setDecompressor(decompressor_ref_t ref) { decompressor = ref; }

    const compressor_t &getCompressor() const { return *compressor; }

    void setCompressor(compressor_ref_t ref) { compressor = ref; }

    void addImage(const Compat::UnixPath &id, Magick::Image img) {
        for (sheet_iterator sheet_it = sheets.begin(); sheet_it != sheets.end();
             ++sheet_it) {
        ...
        if (!pushSheet().addImage(id, img)) {
            throw KToolsError(
                fmt("An atlas texture file has a maximum size of %ux%u. "
                    "Impossible to fit image '%s' of size %ux%u.",
                    (unsigned)Sheet::MAX_WIDTH, (unsigned)Sheet::MAX_HEIGHT,
                    id.c_str(), (unsigned)img.columns(), (unsigned)img.rows()));
        }
    }
    ...
    sheet_const_iterator begin() const { return sheets.begin(); }

    sheet_const_iterator end() const { return sheets.end(); }

    std::istream &load(std::istream &in, const VirtualPath &p,
                       int verbosity = -1) {
        setPath(p);
    ...
    std::ostream &dump(std::ostream &out, const VirtualPath &p,
                       int verbosity = -1) {
        setPath(p);
    ...
    void saveImages(const VirtualPath &output_dir, bool clear_on_done,
        ...
        for (sheet_const_iterator sheet_it = begin(); sheet_it != end();
             ++sheet_it) {
