...
class VirtualDirectory : public Compat::Path {
  ...
  public:
    ...
    typedef std::istream *(*in_handler_t)(const VirtualDirectory &,
                                          const Compat::Path &subpath,
                                          ios_base::openmode);
    typedef std::ostream *(*out_handler_t)(const VirtualDirectory &,
                                           const Compat::Path &subpath,
                                           ios_base::openmode);
  ...
  private:
    VirtualDirectory(const Compat::Path &p, Type t)
        : Compat::Path(p), type(t) {}
    VirtualDirectory(const VirtualDirectory &d)
        : Compat::Path(d), type(d.type) {}
  ...
  public:
    ...
    std::istream *
    open_in(const Compat::Path &subpath,
            ios_base::openmode mode = ios_base::openmode()) const {
        mode |= ios_base::in;
        return getHandlers().first(*this, subpath, mode);
    }

    std::ostream *
    open_out(const Compat::Path &subpath,
             ios_base::openmode mode = ios_base::openmode()) const {
        mode |= ios_base::out;
...
class VirtualPath : public Compat::Path {
  ...
  private:
    ...
    static bool zipEntryExists(const VirtualDirectory &zippath,
  ...
  public:
    ...
    std::istream *
    open_in(std::ios_base::openmode mode = std::ios_base::openmode()) const {
        Compat::Path subpath;
    ...
    // The pointer should later be deallocated by the user.
    std::ostream *
    open_out(std::ios_base::openmode mode = std::ios_base::openmode()) const {
        Compat::Path subpath;
        VirtualDirectory d = getVirtualDirectory(subpath);
