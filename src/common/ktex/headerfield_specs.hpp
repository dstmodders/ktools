...
struct HeaderFieldSpec {
    ...
    value_t normalize_value(value_t v) const { return v; }
    ...
    const std::string &normalize_value_inverse(value_t v) const {
        ...
        std::map<value_t, std::string>::const_iterator it =
            values_inverse.find(v);
  ...
  private:
    void ConstructBasic(const std::string &_id, const std::string &_name,
                        unsigned int _length, value_t _value_default) {
        id = _id;
        name = _name;
        length = _length;
    ...
    void ConstructValues(const raw_pair<std::string, value_t> *_value_pairs,
                         size_t nvalues) {
        for (size_t i = 0; i < nvalues; i++) {
            values.insert(
                std::make_pair(_value_pairs[i].first, _value_pairs[i].second));
            values_inverse.insert(
                std::make_pair(_value_pairs[i].second, _value_pairs[i].first));
        }
    }

    void Construct(const std::string &_id, const std::string &_name,
                   unsigned int _length,
                   const raw_pair<std::string, value_t> *_value_pairs,
                   size_t nvalues, value_t _value_default) {
    ...
    void Construct(const std::string &_id, const std::string &_name,
                   unsigned int _length,
                   const raw_pair<std::string, value_t> *_value_pairs,
                   size_t nvalues, const std::string &value_default_name) {
  ...
  public:
    HeaderFieldSpec(const std::string &_id, const std::string &_name,
                    unsigned int _length,
                    const raw_pair<std::string, value_t> *_value_pairs,
                    size_t nvalues, value_t _value_default = value_t()) {
    ...
    HeaderFieldSpec(const std::string &_id, const std::string &_name,
                    ...
                    const raw_pair<std::string, value_t> *_value_pairs,
                    size_t nvalues, const std::string &value_default_name) {
        Construct(_id, _name, _length, _value_pairs, nvalues,
                  value_default_name);
    }

    HeaderFieldSpec(const std::string &_id, const std::string &_name,
                    unsigned int _length, value_t _value_default = value_t()) {
        ConstructBasic(_id, _name, _length, _value_default);
    }
};
} // namespace KTEX
