...
void KTools::KTEX::File::loadFrom(const std::string &path, int verbosity,
                                  bool info_only) {
    if (verbosity >= 0) {
...
void KTools::KTEX::File::Header::print(std::ostream &out, int verbosity,
                                       size_t indentation,
    ...
    for (field_spec_offset_iterator it = FieldSpecs.offset_begin();
         it != FieldSpecs.offset_end(); ++it) {
...
std::istream &KTools::KTEX::File::Header::load(std::istream &in) {
    ...
    // conclusion.
    io.setLittleSource();
    if ((getField("fill") & 0xff) ==
        (FieldSpecs["fill"].value_default & 0xff)) {
        io.setNativeSource();
    } else {
        Header header_cp = *this;
        io.reorder(header_cp.data);
        if ((header_cp.getField("fill") & 0xff) ==
            (FieldSpecs["fill"].value_default & 0xff)) {
...
void KTools::KTEX::File::Mipmap::print(std::ostream &out, size_t indentation,
                                       const std::string &indent_string) const {
    using namespace std;
...
void KTools::KTEX::File::print(std::ostream &out, int verbosity,
                               size_t indentation,
                               const std::string &indent_string) const {
    using namespace std;
...
std::ostream &KTools::KTEX::File::dump(std::ostream &out, int verbosity) const {
    ...
    for (size_t i = 0; i < mipmap_count; ++i) {
        if (verbosity >= 1) {
            std::cout << "Dumping (pre) mipmap #" << (i + 1) << "..."
                      << std::endl;
            if (verbosity >= 2) {
    ...
    for (size_t i = 0; i < mipmap_count; ++i) {
        if (verbosity >= 1) {
            std::cout << "Dumping (post) mipmap #" << (i + 1) << "..."
                      << std::endl;
        }
...
std::istream &KTools::KTEX::File::load(std::istream &in, int verbosity,
                                       bool info_only) {
    BinIOHelper::sanitizeStream(in);
    ...
    for (size_t i = 0; i < mipmap_count; ++i) {
        if (verbosity >= 1) {
            std::cout << "Loading (pre) mipmap #" << (i + 1) << "..."
                      << std::endl;
        }
    ...
    for (size_t i = 0; i < mipmap_count; ++i) {
        if (verbosity >= 1) {
            std::cout << "Loading (post) mipmap #" << (i + 1) << "..."
                      << std::endl;
        }
    ...
    if (in.peek() != EOF) {
        std::cerr << "Warning: There is leftover data in the input TEX file."
                  << std::endl;
    }
...
KTools::KTEX::File::CompressionFormat
KTools::KTEX::File::getCompressionFormat() const {
    KTools::KTEX::File::CompressionFormat fmt;
    fmt.squish_flags = getSquishCompressionFlag(header, fmt.is_uncompressed);
    return fmt;
}

Magick::Image KTools::KTEX::File::DecompressMipmap(
    const KTools::KTEX::File::Mipmap &M,
    const KTools::KTEX::File::CompressionFormat &fmt, int verbosity) const {
    Magick::Image img;
    Magick::Blob B;
    ...
    if (!fmt.is_uncompressed) {
        if (verbosity >= 0) {
            std::cout << "Decompressing " << width << "x" << height
                      << " KTEX image into RGBA..." << std::endl;
        }
        squish::u8 *RESTRICT rgba = new squish::u8[4 * width * height];
        squish::DecompressImage(rgba, width, height, M.getData(),
                                fmt.squish_flags);
...
void KTools::KTEX::File::CompressMipmap(
    KTools::KTEX::File::Mipmap &M,
    const KTools::KTEX::File::CompressionFormat &fmt, Magick::Image img,
    int verbosity) const {
    (void)verbosity;
    ...
    if (fmt.is_uncompressed) {
        ...
        cast_assign(M.pitch, squish::GetStorageRequirements(int(width), 1,
                                                            fmt.squish_flags));
    }
    ...
    if (fmt.is_uncompressed) {
        ...
        M.setDataSize(squish::GetStorageRequirements(int(width), int(height),
                                                     fmt.squish_flags));
    }
    ...
    if (fmt.is_uncompressed) {
        ...
        squish::CompressImage((const squish::u8 *)B.data(), int(width),
                              int(height), M.data, fmt.squish_flags);
    }
}
