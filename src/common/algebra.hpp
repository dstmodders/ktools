...
template <size_t n, typename T> class ProjectiveMatrix;

template <size_t N, typename T = float_type> class Vector {
    template <size_t M, typename U> friend class Vector;

    template <size_t M, typename U> friend class SquareMatrix;

    template <size_t M, typename U> friend class ProjectiveMatrix;
  ...
  public:
    ...
    Vector operator*(T lambda) const { return Vector(*this) *= lambda; }

    friend Vector operator*(T lambda, const Vector &v) {
        return Vector(v) *= lambda;
    }

    Vector &negate() { return (*this) *= -1; }

    Vector operator-() const { return Vector(*this).negate(); }
    ...
    // inner
    T operator*(const Vector &v) const { return inner(v); }

    template <size_t M> Vector<M, T> extend(T pad = T()) const {
        Vector<M, T> v = nil;
...
class SquareMatrix : public Vector<n * n, T> {
  ...
  public:
    ...
    SquareMatrix() : super() { setDiagonal(1); }
    SquareMatrix(Nil) : super(nil) {}
    explicit SquareMatrix(T diag) : super() { setDiagonal(diag); }
    ...
    SquareMatrix copy() const { return SquareMatrix(*this); }

    static size_t rows() { return n; }

    static size_t columns() { return n; }

    T *operator[](size_t i) { return (this->x + n * i); }

    const T *operator[](size_t i) const { return (this->x + n * i); }
  ...
  protected:
    static void multiply(const SquareMatrix &A, const SquareMatrix &B,
                         SquareMatrix &result) {
        for (size_t i = 0; i < n; i++) {
            const T *Arow = A[i];
            T *Rrow = result[i];
  ...
  public:
    ...
    vector_type operator()(const vector_type &v) const { return (*this) * v; }
};
 ...
 */
template <typename T> inline void raw_invert(T &a, T &b, T &c, T &d) {
    T invdet = 1 / (a * d - c * b);
...
template <typename T> inline void invert(SquareMatrix<2, T> &M) {
...
template <typename T> inline void invert(ProjectiveMatrix<2, T> &M) {
    Vector<2, T> v;
    M.getTranslation(v);
...
template <typename T = float_type, size_t N = 3> class Triangle {
  public:
    ...
    Triangle() : a(), b(), c() {}
    Triangle(const vertex_type &_a, const vertex_type &_b,
             const vertex_type &_c)
        : a(_a), b(_b), c(_c) {}
...
template <size_t coord, typename bbox_t> class BoundingBoxAccessor {};

template <typename bbox_t> class BoundingBoxAccessor<0, bbox_t> {
  public:
    typedef typename bbox_t::scalar_type scalar_type;

    static inline scalar_type min(const bbox_t &bbox) { return bbox.x(); }

    static inline scalar_type max(const bbox_t &bbox) { return bbox.xmax(); }
};

template <typename bbox_t> class BoundingBoxAccessor<1, bbox_t> {
  public:
    typedef typename bbox_t::scalar_type scalar_type;

    static inline scalar_type min(const bbox_t &bbox) { return bbox.y(); }

    static inline scalar_type max(const bbox_t &bbox) { return bbox.ymax(); }
};

template <typename T = float_type> class BoundingBox {
  public:
  ...
  private:
    ...
    static int dim_to_int(float val) { return int(ceilf(val)); }

    static int dim_to_int(double val) { return int(ceil(val)); }
  ...
  public:
    ...
    BoundingBox(T _x, T _y, T _w, T _h) { setDimensions(_x, _y, _w, _h); }

    BoundingBox(const BoundingBox &bbox)
        : bottom_left(bbox.bottom_left), top_right(bbox.top_right),
    ...
    const vector_type &bottomLeft() const { return bottom_left; }

    const vector_type &topRight() const { return top_right; }

    T x() const { return bottom_left[0]; }

    void x(T _x) { bottom_left[0] = _x; }

    T xmax() const { return top_right[0]; }

    void xmax(T _x) { top_right[0] = _x; }

    T y() const { return bottom_left[1]; }

    void y(T _y) { bottom_left[1] = _y; }

    T ymax() const { return top_right[1]; }

    void ymax(T _y) { top_right[1] = _y; }

    T w() const { return top_right[0] - bottom_left[0]; }

    void w(T _w) { top_right[0] = bottom_left[0] + _w; }

    T h() const { return top_right[1] - bottom_left[1]; }

    void h(T _h) { top_right[1] = bottom_left[1] + _h; }

    int int_w() const { return dim_to_int(w()); }

    int int_h() const { return dim_to_int(h()); }
    ...
    template <size_t N> void addPoint(const Vector<N, T> &v) {
        staticAssert<N >= 2>();
        addPoint(v[0], v[1]);
    }

    template <size_t N> void addPoints(const Triangle<T, N> &t) {
        addPoint(t.a);
        addPoint(t.b);
        addPoint(t.c);
    }
