...
#error Unsupported native endianness.
    ...
    static inline void read_fail() {
        throw KToolsError(
            "failed to read data from stream. It might be corrupt.");
    }
    ...
    template <typename T> static void raw_read_value(std::istream &in, T &n) {
  ...
  public:
    BinIOHelper()
        : source_endianness(ktools_UNKNOWN_ENDIAN),
          target_endianness(ktools_LITTLE_ENDIAN) {}

    void setNativeSource() { source_endianness = ktools_NATIVE_ENDIANNESS; }
    void setInverseNativeSource() {
        source_endianness = ktools_INVERSE_NATIVE_ENDIANNESS;
    }
    void setLittleSource() { source_endianness = ktools_LITTLE_ENDIAN; }
    void setBigSource() { source_endianness = ktools_BIG_ENDIAN; }
    ...
    bool isBigSource() const { return source_endianness == ktools_BIG_ENDIAN; }
    bool isUnknownSource() const {
        return source_endianness == ktools_UNKNOWN_ENDIAN;
    }

    void setNativeTarget() { target_endianness = ktools_NATIVE_ENDIANNESS; }
    void setInverseNativeTarget() {
        target_endianness = ktools_INVERSE_NATIVE_ENDIANNESS;
    }
    void setLittleTarget() { target_endianness = ktools_LITTLE_ENDIAN; }
    void setBigTarget() { target_endianness = ktools_BIG_ENDIAN; }
    ...
    bool isBigTarget() const { return target_endianness == ktools_BIG_ENDIAN; }
    ...
    template <typename T> static void raw_read_integer(std::istream &in, T &n) {
    ...
    template <typename T> static void reorder(T &n) {
    ...
    template <typename T> inline void read_float(std::istream &in, T &x) const {
    ...
    static inline void read_string(std::istream &in, std::string &str,
                                   const size_t len) {
        char buffer[1 << 15];
        if (len > sizeof(buffer)) {
            throw KToolsError(
                strformat("Attempt to read a string larger than %lu bytes. The "
                          "stream might be corrupt.",
    ...
    */
    ...
    template <typename LengthType>
    inline void write_len_string(std::ostream &out,
    ...
    static inline void openBinaryStream(std::ifstream &in,
    ...
    static inline void openBinaryStream(std::ofstream &out,
                                        const std::string &fname,
                                        bool validate = true) {
    ...
    static inline uint32_t getMagicNumber(std::istream &in,
                                          bool rewind = true) {
        std::streampos p;
    ...
    static inline uint32_t getMagicNumber(const std::string &fname,
                                          bool rewind = true) {
        std::ifstream in;
