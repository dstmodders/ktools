...
class Error : public std::exception {
  ...
  protected:
    void setMessage(const std::string &cppwhat_) { cppwhat = cppwhat_; }
...
template <typename T, typename U> struct raw_pair {
    T first;
    U second;
};
...
class lcstring {
  public:
    ...
    inline const char *operator()(void) const { return data; }

    inline size_t length() const { return len; }

    inline size_t size() const { return len; }

    inline std::string tostring() const { return std::string(data, len); }
};

template <typename T, typename U> inline T &cast_assign(T &lval, U rval) {
...
template <typename T> class Maybe {
  ...
  public:
    ...
    bool operator==(Nil) const { return is_nothing; }

    friend inline bool operator==(Nil, const Maybe &m) { return m.is_nothing; }

    bool operator==(const Maybe &m) const {
        return (is_nothing && m.is_nothing) ||
               (!is_nothing && !m.is_nothing && v == m.v);
    }
    ...
    template <typename U> friend bool operator==(const U &u, const Maybe &m) {
        return m == u;
    }

    template <typename U> bool operator==(const Maybe<U> &m) const {
        return is_nothing && m.is_nothing;
    }
    ...
    T val() const { return to<T>(); }

    T &ref() { return to<T &>(); }

    const T &ref() const { return to<const T &>(); }

    operator T() const { return value(); }
};

template <typename T> Maybe<T> Just(T val) { return Maybe<T>(val); }
...
inline bool check_basic_stream_validity(std::basic_ios<charT, charTraits> &file,
...
inline bool check_stream_validity(std::istream &in,
                                  const std::string &fname = "file",
                                  bool _throw = true) {
    (void)in.peek();
    return check_basic_stream_validity(
        in, "Failed to open " + fname + " for reading", _throw);
}

inline bool check_stream_validity(std::ostream &out,
                                  const std::string &fname = "file",
                                  bool _throw = true) {
    return check_basic_stream_validity(
        out, "Failed to open " + fname + " for writing", _throw);
}
} // namespace KTools

#define MAGICK_WRAP(code)                                                      \
    try {                                                                      \
        code;                                                                  \
