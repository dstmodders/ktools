...
class Output : public TCLAP::StdOutput {
    ...
    typedef std::map<categories_list_t::value_type, size_t>
        categories_inverse_t;
    ...
    void getSortedArgs(const std::list<TCLAP::Arg *> &args,
                       std::vector<TCLAP::Arg *> &sorted_args) const {
        sorted_args.clear();

        for (std::list<TCLAP::Arg *>::const_iterator it = args.begin();
             it != args.end(); ++it) {
            if (!isUnlabeled(*it)) {
...
template <typename T> class StrOptTranslator {
  public:
    // List of options
...
class ArgumentOption : public UnlabeledValueArg<std::string> {
  ...
  public:
    void setVisuallyRequired(bool b) { visual_required_override = Just(b); }
    ...
    virtual std::string shortID(const std::string &val = "") const {
        ...
        bool required = (visual_required_override == nil
                             ? isRequired()
                             : visual_required_override.value());
    ...
    ArgumentOption(const std::string &name, const std::string &desc, bool req,
...
template <typename T> class MyValueArg : public ValueArg<T> {
  public:
    MyValueArg(const std::string &flag, const std::string &name,
               const std::string &desc, bool req, T value,
               const std::string &typeDesc)
        : ValueArg<T>(flag, name, desc, req, value, typeDesc) {}

    MyValueArg(const std::string &flag, const std::string &name,
               const std::string &desc, bool req, T value,
               Constraint<T> *constraint)
        : ValueArg<T>(flag, name, desc, req, value, constraint) {}

    virtual std::string longID(const ::std::string &val = "") const;
};

template <typename T> class MyMultiArg : public MultiArg<T> {
  public:
    MyMultiArg(const std::string &flag, const std::string &name,
               const std::string &desc, bool req, const std::string &typeDesc)
        : MultiArg<T>(flag, name, desc, req, typeDesc) {}

    MyMultiArg(const std::string &flag, const std::string &name,
               const std::string &desc, bool req, Constraint<T> *constraint)
        : MultiArg<T>(flag, name, desc, req, constraint) {}
...
inline void
KTools::options_custom::Output::version(TCLAP::CmdLineInterface &c) {
...
inline void
KTools::options_custom::Output::usage_short(TCLAP::CmdLineInterface &c,
                                            std::ostream &out) {
    using namespace std;
    using namespace TCLAP;
...
inline void
KTools::options_custom::Output::usage_long(TCLAP::CmdLineInterface &c,
                                           std::ostream &out) {
    using namespace std;
    using namespace TCLAP;
    ...
    for (std::vector<Arg *>::const_iterator it = sorted_args.begin();
         it != sorted_args.end(); ++it) {
...
inline void KTools::options_custom::Output::failure(TCLAP::CmdLineInterface &c,
...
template <typename T>
inline std::string
KTools::options_custom::MyValueArg<T>::longID(const ::std::string &val) const {
    using namespace std;
    using namespace TCLAP;
    (void)val;
    ...
    if (this->isValueRequired()) {
        id += string(2, Arg::delimiter()) +
              std::string(1, option_brackets[1][0]) + valueId +
              std::string(1, option_brackets[1][1]);
    }
...
template <typename T>
inline std::string
KTools::options_custom::MyMultiArg<T>::longID(const ::std::string &val) const {
    using namespace std;
    using namespace TCLAP;
    (void)val;
