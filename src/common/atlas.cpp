...
template <int (*f)(int)> static std::string &mapstr(std::string &s) {
...
xml_attribute require_attr(xml_node node, const char *name) {
    ...
    if (!ret) {
        throw KTools::KToolsError(std::string("required attribute '") + name +
                                  "' missing.");
    }
    return ret;
}
...
void AtlasSheet::synthesize() const {
    ...
    for (const_iterator it = images.begin(); it != images.end();
         ++it, ++bbox_it) {
        final_image.composite(it->second, bbox_it->x(), bbox_it->y(),
...
void AtlasSheet::analyze_image(AtlasSheet::imagelist_t::iterator img_it,
                               bbox_list_t::const_iterator bbox_it) const {
    img_it->second = final_image;
    Magick::Geometry geo(bbox_it->w(), bbox_it->h(), bbox_it->x(),
                         bbox_it->y());
    // cerr << "+" << bbox_it->x() << "+" << bbox_it->y() << " " << bbox_it->w()
    // << "x" << bbox_it->h() << endl;
...
void AtlasSheet::load(xml_node node, const VirtualPath &basedir,
                      int verbosity) {
    Compat::UnixPath texture_filename =
        require_attr(node, "filename").as_string();
    setSubPath(texture_filename);

    try {
        if (verbosity >= 1) {
            std::cout << "Loading atlas texture file data for '"
        ...
        for (xml_node el = ichild(elements_node, "Element"); el;
             el = isibling(el, "Element")) {
        ...
        throw KToolsError(std::string("under texture file '") + getSubPath() +
                          "': " + err.what());
    }
}

void AtlasSheet::dump(xml_node node, const VirtualPath &basedir,
                      int verbosity) const {
    const std::string &texture_filename = getSubPath();

    try {
        if (verbosity >= 1) {
            std::cout << "Saving atlas texture file data for '"
                      << texture_filename << "'" << std::endl;
        }
        ...
        xml_node elements_node = node.parent().append_child("Elements");
        {
            ...
            for (img_it = images.begin(), bbox_it = bboxes.begin();
                 img_it != images.end(); ++img_it, ++bbox_it) {
...
void AtlasSheet::saveImages(const VirtualPath &output_dir, bool clear_on_done,
                            int verbosity) const {
    iterator it = images.begin();
    for (bbox_list_t::const_iterator bbox_it = bboxes.begin();
         bbox_it != bboxes.end(); ++bbox_it, ++it) {
        ...
        Compat::Path output_path =
            (output_dir / it->first).replaceExtension("png", true);
        if (verbosity >= 3) {
...
std::istream &Atlas::do_load(std::istream &in, int verbosity) {
    ...
    try {
        if (verbosity >= 0) {
            std::cout << "Loading atlas from '" << getPath() << "'..."
                      << std::endl;
        }
        ...
        xml_parse_result result =
            atlas_file_doc.load(in, parse_default, encoding_utf8);
        ...
        for (xml_node child = ichild(atlas_node, "Texture"); child;
             child = isibling(child, "Texture")) {
        ...
        if (!found_any_texture) {
            std::cerr
                << "WARNING: the atlas '" << getPath()
                << "' does not specify any texture files. No output generated."
                << std::endl;
        }

        if (verbosity >= 0) {
            std::cout << "Loaded atlas from '" << getPath() << "'."
                      << std::endl;
        }
    } catch (const KToolsError &err) {
        throw KToolsError(std::string("Failed to load xml document '") +
                          getPath() + "': " + err.what());
    }
...
std::ostream &Atlas::do_dump(std::ostream &out, int verbosity) const {
    ...
    try {
        if (verbosity >= 0) {
            std::cout << "Saving atlas to '" << getPath() << "'..."
                      << std::endl;
        }
        ...
        for (sheet_const_iterator it = sheets.begin(); it != sheets.end();
             ++it) {
        ...
        throw KToolsError(std::string("Failed to save xml document '") +
                          getPath() + "': " + err.what());
    }
