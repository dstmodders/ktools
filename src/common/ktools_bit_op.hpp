...
namespace BitOp {
class Pow2Rounder {
    template <typename IntegerType> class Metadata {
      public:
         ...
         */
        static bool isPow2(IntegerType n) { return !(n & (n - 1)); }
    };
  ...
  public:
    ...
    template <typename IntegerType> static IntegerType roundUp(IntegerType n) {
...
template <typename IntegerType> inline size_t countBinaryDigits(IntegerType n) {
    size_t ret = 0;
    while (n) {
        n >>= 1;
        ret++;
...
template <size_t offset, size_t len, typename IntegerType> class BitMask {
  public:
    static const IntegerType value =
        (1 << offset) | BitMask<offset + 1, len - 1, IntegerType>::value;
};
...
template <typename IntegerType> class BitQueue {
    IntegerType n;
