...
namespace KTools {
template <bool condition> class StaticAssertionChecker {
    char static_assertion_failed[condition ? 1 : -1];
};

template <bool condition> inline void staticAssert() {
...
template <typename T> struct IsFloating : public FalseType {};

template <> struct IsFloating<float> : public TrueType {};

template <> struct IsFloating<double> : public TrueType {};

template <typename T> inline void staticAssertIntegral() {
    staticAssert<IsIntegral<T>::value>();
}

template <typename T> inline void staticAssertFloating() {
