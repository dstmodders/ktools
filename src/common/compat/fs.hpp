...
static const char NATIVE_DUAL_DIRECTORY_SEPARATOR =
    (NATIVE_DIRECTORY_SEPARATOR == '/' ? '\\' : '/');

template <char sep> class PathAbstraction : public std::string {
  public:
  ...
  public:
    ...
    PathAbstraction &operator=(const char *str) { return assignPath(str); }
    ...
    PathAbstraction(const std::string &str) : std::string() { *this = str; }

    PathAbstraction(const char *str) : std::string() { *this = str; }

    PathAbstraction(const PathAbstraction &p) : std::string() { *this = p; }

    PathAbstraction() {}

    virtual ~PathAbstraction() {}

    PathAbstraction copy() const { return PathAbstraction(*this); }
    ...
    PathAbstraction &operator/=(const char *str) { return appendPath(str); }
    ...
    bool hasExtension() const { return get_extension_position() != npos; }
