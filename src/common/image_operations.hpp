...
typedef std::unary_function<MagickCore::Quantum *, void>
    basic_pixel_operation_t;
...
class operation_t : public std::unary_function<argument_type, result_type> {
  public:
    ...
    */
    ...
    inline result_type operator()(argument_type arg) const { return call(arg); }

    template <typename T> inline result_type operator()(T &arg) const {
        return call(arg);
    }
};

template <typename first_argument_type, typename second_argument_type>
class binary_operation_t
    : public std::binary_function<first_argument_type, second_argument_type,
                                  void> {
  public:
    typedef std::binary_function<first_argument_type, second_argument_type,
                                 void>
        basic_operation_t;
    ...
    virtual void call(first_argument_type arg1,
                      second_argument_type arg2) const = 0;

    inline void operator()(first_argument_type arg1,
...
template <class Op> class reference_caller {};

template <typename argument_type, typename result_type>
class reference_caller<operation_t<argument_type, result_type> >
    : public operation_t<argument_type, result_type> {
  public:
    ...
    virtual result_type call(argument_type arg) const { return docall(arg); }
};

template <typename Arg1, typename Arg2>
class reference_caller<binary_operation_t<Arg1, Arg2> >
    : public binary_operation_t<Arg1, Arg2> {
  public:
    typedef binary_operation_t<Arg1, Arg2> basic_operation;

    virtual const basic_operation *get_pointer() const = 0;

    void docall(Arg1 arg1, Arg2 arg2) const { get_pointer()->call(arg1, arg2); }

    virtual void call(Arg1 arg1, Arg2 arg2) const { docall(arg1, arg2); }
};

///

template <typename basic_operation>
class operation_ref_t
    : public reference_caller<typename basic_operation::type> {
    Magick::Blob ref;
    ...
    void copy_data(const basic_operation *op, size_t sz) { ref.update(op, sz); }
  ...
  public:
    ...
    inline const basic_operation &deference() const { return *get_pointer(); }

    const basic_operation &operator*() const { return deference(); }

    const basic_operation *operator->() const { return get_pointer(); }

    operator const basic_operation &() const { return deference(); }

    inline bool empty() const { return ref.length() == 0; }

    inline operator bool() const { return !empty(); }

    inline bool operator==(Nil) const { return empty(); }
};
...
class read : public unary_operation_t {
  ...
  public:
    ...
    virtual void call(Magick::Image &img) const { img.read(path); }
};
...
inline MagickCore::Quantum multiplyQuantum(MagickCore::Quantum q,
                                           double factor) {
...
template <typename PixelOperation> class pixelMap : public image_operation_t {
    PixelOperation op;
