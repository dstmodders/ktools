  ...
  public:
    ...
    class Symbol : public NestedSerializer<KBuild> {
      ...
      public:
        class Frame : public NestedSerializer<Symbol> {
          ...
          private:
            ...
            /*
             * This is the time length of this frame measured in animation
             * frames.
             */
            uint32_t duration;
            ...
            int getAtlasDepth() const { return atlas_depth; }

          public:
            uint32_t getFrameNumber() const { return framenum; }

            uint32_t getDuration() const { return duration; }

            bbox_type &getBoundingBox() { return bbox; }

            const bbox_type &getBoundingBox() const { return bbox; }
            ...
            /*
             * Bounding box of the corresponding atlas region, in UV
             * coordinates.
             */
            BoundingBox<float_type> atlas_bbox;

            void addTriangles(const xyztriangle_type &xyz,
                              const uvwtriangle_type &uvw) {
            ...
            uint32_t countAlphaVerts() const { return 3 * countTriangles(); }
            ...
            void getName(std::string &s) const {
                ...
                strformat_inplace(s, "%s-%u", parent_name.c_str(),
      ...
      private:
        ...
        void update_framenumbermap() {
            ...
            for (uint32_t i = 0; i < nframes; i++) {
                frameNumberMap.insert(
                    std::make_pair(frames[i].getFrameNumber(), i));
            }
        }
      ...
      public:
        ...
        int getSamplerOffset() const { return parent->getSamplerOffset(); }

        /*
         * Receives a frame number (as in the attribute frame number of Frame)
         * and returns the index in the frames vector of the corresponding
         * frame.
         */
        uint32_t getFrameIndex(uint32_t framenum) const {
            if (frameNumberMap.empty()) {
                throw std::logic_error(
                    "querying an empty build symbol frame number map.");
            }
            frameNumberMap_t::const_iterator match =
                frameNumberMap.upper_bound(framenum);
        ...
        // animation states).
        uint32_t countFrames() const { return frames.size(); }
        ...
        void getPath(Compat::Path &p) const { p.assign(getName()); }
        ...
        void getUnixPath(Compat::UnixPath &p) const { p.assign(getName()); }
    ...
    */
    ...
    int getSamplerOffset() const {
        if (sampler_offset == nil) {
            ...
            for (symbolmap_t::const_iterator symit = symbols.begin();
                 symit != symbols.end(); ++symit) {
                const Symbol::framelist_t &frames = symit->second.frames;
                for (Symbol::framelist_t::const_iterator fit = frames.begin();
                     fit != frames.end(); ++fit) {
            ...
            if (max_depth - sampler_offset + 1 > int(atlases.size())) {
                throw KToolsError("Build has symbol frames requesting atlases "
                                  "the build does not possess.");
            }
        }
    ...
    template <typename OutputIterator> void getImages(OutputIterator it) const {
        typedef symbolmap_t::const_iterator symbol_iter;
        typedef Symbol::framelist_t::const_iterator frame_iter;

        for (symbol_iter symit = symbols.begin(); symit != symbols.end();
             ++symit) {
            for (frame_iter frit = symit->second.frames.begin();
                 frit != symit->second.frames.end(); ++frit) {
    ...
    void getClipMasks(OutputIterator it) const {
        size_t idx = 0;
        for (atlaslist_t::const_iterator atit = atlases.begin();
             atit != atlases.end(); ++atit) {
            *it++ = getClipMask(idx++);
        }
    }
...
class KBuildFile : public NonCopyable {
  ...
  public:
    ...
    void versionRequire() const {
        if (!checkVersion()) {
            std::cerr << "Build has unsupported encoding version " << version
                      << "." << std::endl;
            throw(EncodingVersionError(
                "Build has unsupported encoding version."));
        }
    }

    int32_t getVersion() const { return version; }

    KBuild *getBuild() { return build; }

    const KBuild *getBuild() const { return build; }

    void setBuild(KBuild *b) { build = b; }

    // Clears ownership without deleting anything.
    void softClear() { setBuild(NULL); }
    ...
    operator bool() const { return build != NULL; }
