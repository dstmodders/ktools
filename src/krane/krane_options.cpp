...
void Krane::parse_commandline_options(
    int &argc, char **&argv, std::list<KTools::VirtualPath> &input_paths,
    ...
    try {
        ...
        /*
        SwitchArg info_flag("i", "info", "Prints information for the given input
        files instead of performing any conversion.");
        args.push_back(&info_flag);
        */

        MyValueArg<string> allowed_build_opt(
            "", "build", "Selects only a build with the given name.", false, "",
            "build name");
        args.push_back(&allowed_build_opt);
        myOutput.setArgCategory(allowed_build_opt, INPUT_CTRL);

        MyMultiArg<string> allowed_banks_opt(
            "", "bank", "Selects only animations in the given banks.", false,
            "bank name");
        args.push_back(&allowed_banks_opt);
        myOutput.setArgCategory(allowed_banks_opt, INPUT_CTRL);

        SwitchArg mark_atlas_opt(
            "", "mark-atlases",
        ...
        MyValueArg<string> build_rename_opt(
            "", "rename-build", "Renames the input build to the given name.",
            false, "", "build name");
        args.push_back(&build_rename_opt);
        // myOutput.setArgCategory(build_rename_opt, TO_SCML);

        MyValueArg<string> banks_rename_opt(
            "", "rename-bank", "Renames the input banks to the given name.",
            false, "", "bank name");
        args.push_back(&banks_rename_opt);
        // myOutput.setArgCategory(banks_rename_opt, TO_SCML);

        SwitchArg check_anim_fidelity_opt(
            "", "check-animation-fidelity",
            "Checks if the Spriter representation of the animations is "
            "faithful to the source animations.");
         ...
         * multiinput_opt.
         */
        ArgumentOption dummy_output_opt("OUTPUT-PATH", "Output path.", false,
                                        "OUTPUT-DIR");
        ...
        if (allowed_banks_opt.isSet()) {
            ...
            options::allowed_banks.insert(options::allowed_banks.begin(),
        ...
        */
        ...
        if (all_paths.size() < 2) {
            ...
            cmd.parse(argc, argv);
            // This should never be throws, because the above cmd.parse() is
            // meant to raise an error and exit the program.
