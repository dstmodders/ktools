...
Magick::Image KBuild::Symbol::Frame::getClipMask() const {
    ...
    for (size_t i = 0; i < ntrigs; i++) {
        ...
        coords.push_back(Coordinate((trig.a[0] - atlas_bbox.x()) * w0,
                                    (trig.a[1] - atlas_bbox.y()) * h0));
        coords.push_back(Coordinate((trig.b[0] - atlas_bbox.x()) * w0,
                                    (trig.b[1] - atlas_bbox.y()) * h0));
        coords.push_back(Coordinate((trig.c[0] - atlas_bbox.x()) * w0,
                                    (trig.c[1] - atlas_bbox.y()) * h0));
...
Magick::Image KBuild::Symbol::Frame::getImage() const {
    ...
    float_type w0, h0;
    {
        ...
        try {
            ...
            cerr << w.what() << endl;
            cerr << "When cropping " << int(w0) << "x" << int(h0) << " atlas "
                 << parent->parent->atlases[getAtlasIdx()].first << " to "
                 << geo.width() << "x" << geo.height() << "+" << geo.xOff()
                 << "+" << geo.yOff() << " to select image " << getUnixPath()
                 << endl;
        }
    }
    ...
    // This is to reverse the scaling down applied by the mod tools' scml
    // compiler.
    ...
    try {
        ...
        cerr << w.what() << endl;
        cerr << "When reszing " << img.columns() << "x" << img.rows()
             << " image " << getUnixPath() << " to " << scaling_geo.width()
             << "x" << scaling_geo.height() << endl;
    }
    ...
    for (symbol_iter symit = symbols.begin(); symit != symbols.end(); ++symit) {
        for (frame_iter frit = symit->second.frames.begin();
            ...
            if (size_t(frit->getAtlasIdx()) == idx) {
                ...
                MAGICK_WRAP(
                    mask.composite(framemask, framegeo, OverCompositeOp));
            }
        }
    }
