...
static void preprocess_input_list(pathlist_t &inputs) {
    ...
    for (iter_t it = inputs.begin(); it != inputs.end();) {
        if (!it->exists()) {
            cerr << "Input path `" << *it << "' does not exist, skipping..."
                 << endl;
            inputs.erase(it++);
            continue;
        }
...
static std::string append_sentence(const std::string &original,
...
static KBuild *load_build(std::istream &in,
                          const pathlist_t::value_type &inputdir) {
    ...
    try {
        ...
        cerr << "WARNING: " << append_sentence(e.what(), "Skipping build file.")
             << endl;
        return NULL;
    }

    KBuild *bild = bildfile.getBuild();
    if (options::allowed_build != nil &&
        bild->getName() != options::allowed_build.value()) {
        return NULL;
    }

    for (atlasiter_t it = bild->atlases.begin(); it != bild->atlases.end();
         ++it) {
...
static void load_anims(std::istream &in, KAnimBankCollection &banks) {
    ...
    try {
        ...
        cerr << "WARNING: "
             << append_sentence(e.what(), "Skipping animation file.") << endl;
        return;
    }
...
static KBuild *process_input_list(const pathlist_t &inputs,
    ...
    for (iter_t it = inputs.begin(); it != inputs.end(); ++it) {
        ...
        if (magic_number == KBuildFile::MAGIC_NUMBER) {
            ...
            if (newbild == NULL) {
                ...
                delete bild;
                throw KToolsError(
                    "Multiple builds found in the input. Restrict the choice "
                    "by specifying a build name.");
            }
...
static void configure_bank_collection(KAnimBankCollection &banks) {
    ...
    if (options::allowed_banks.size() > 0) {
        ...
        for (options::allowed_banks_t::const_iterator it =
                 options::allowed_banks.begin();
             it != options::allowed_banks.end(); ++it) {
            s->addBank(*it);
        }
        banks.setSelector(s);
    }
...
static void perform_SCML_conversion(const Compat::Path &output_path,
                                    KBuild *bild, KAnimBankCollection &banks) {
    ...
    for (imglist_t::iterator it = imglist.begin(); it != imglist.end(); ++it) {
        ...
        if (!imgoutpath.dirname().mkdir()) {
            throw SysError("Failed to create directory " +
                           imgoutpath.dirname());
        }
...
static void perform_atlas_marking(const Compat::Path &output_path,
                                  KBuild *bild) {
    ...
    bild->getMarkedAtlases(std::back_inserter(markedatlases),
                           Magick::ColorRGB(0.3, 0.3, 0.3));

    for (atlaslist_t::iterator it = markedatlases.begin();
         it != markedatlases.end(); ++it) {
