...
template <template <typename T, typename A> class Container = std::deque,
          typename Alloc = std::allocator<KAnim *> >
class KAnimFile;
...
class KAnim : public NestedSerializer<GenericKAnimFile> {
  ...
  public:
    ...
    static const uint8_t FACING_45S =
        FACING_UPLEFT | FACING_UPRIGHT | FACING_DOWNLEFT | FACING_DOWNRIGHT;
    static const uint8_t FACING_90S =
      ...
      public:
        ...
        class Element : public NestedSerializer<Frame> {
          ...
          public:
            ...
            const matrix_type &getMatrix() const { return M; }

            float_type getAnimSortOrder() const { return z; }
          ...
          private:
            ...
            std::istream &loadPost(std::istream &in, const KAnimNamer &ht,
                                   int verbosity);
        };
        ...
        float_type getDuration() const { return parent->getFrameDuration(); }

        uint32_t countEvents() const { return uint32_t(events.size()); }

        uint32_t countElements() const { return uint32_t(elements.size()); }
  ...
  public:
    ...
    hash_t getBankHash() const { return bank_hash; }
    ...
    uint8_t getFacingByte() const { return facing_byte; }

    uint32_t getFrameCount() const { return countFrames(); }

    float_type getFrameRate() const { return frame_rate; }

    float_type getFrameDuration() const { return 1 / frame_rate; }
    ...
    uint32_t countEvents() const {
        uint32_t n = 0;
        for (framelist_t::const_iterator it = frames.begin();
             it != frames.end(); ++it) {
            n += it->countEvents();
        }
        return n;
    ...
    uint32_t countFrames() const { return uint32_t(frames.size()); }
...
class KAnimBank : public std::map<std::string, KAnim *>, NonCopyable {
  ...
  public:
    ...
    void setName(const std::string &_name) { setName(strhash(_name), _name); }
    ...
    void addAnim(KAnim *A) {
        ...
        if (options::verbosity >= 1) {
            std::cout << "Adding anim '" << fullname << "' to bank '" << name
                      << "'" << std::endl;
        }
        ...
        if (match != end()) {
            std::string msg =
                "Duplicate anim '" + match->first + "' in bank '" + name + "'";
...
class GenericKAnimFile : public NonCopyable {
  ...
  public:
    ...
    bool checkVersion() const { return checkVersion(version); }

    void versionRequire() const {
        if (!checkVersion()) {
            std::cerr << "Animation file has unsupported encoding version "
                      << version << "." << std::endl;
            throw(EncodingVersionError(
                "Animation has unsupported encoding version."));
        }
    }
    ...
    virtual std::istream &loadPre_all_anims(std::istream &in,
                                            int verbosity) = 0;
    virtual std::istream &loadPost_all_anims(std::istream &in,
                                             const KAnimNamer &ht,
                                             int verbosity) = 0;
...
class KAnimFile : public GenericKAnimFile {
  public:
    ...
    virtual std::istream &
    loadPost_all_anims(std::istream &in, const KAnimNamer &ht, int verbosity) {
        for (anim_iterator it = anims.begin(); it != anims.end(); ++it) {
    ...
    operator bool() const { return !anims.empty(); }

    virtual ~KAnimFile() { clear(); }
};
...
class KAnimSelector : public std::unary_function<const KAnim &, bool> {
  public:
    ...
    bool operator()(const KAnim &A) { return select(A); }
...
class KAnimBankSelector : public KAnimSelector {
  ...
  public:
    void addBank(const std::string &name) { bank_set.insert(strhash(name)); }
...
class KAnimMapper : public std::unary_function<KAnim &, void> {
  public:
    virtual void apply(KAnim &A) { (void)A; }

    void operator()(KAnim &A) { apply(A); }
...
class KAnimBankRenamer : public KAnimMapper {
  ...
  public:
    ...
    virtual void apply(KAnim &A) { A.setBank(targname); }
};
...
class KAnimBankCollection : public std::map<hash_t, KAnimBank *>, NonCopyable {
  ...
  public:
    ...
    void setMapper(KAnimMapper *_f) { f = _f; }
    ...
    template <typename Iterator> void addAnims(Iterator first, Iterator last) {
        while (first != last) {
            addAnim(*first++);
        }
    }
