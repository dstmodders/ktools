...
template <typename T> static inline T normalize_time(T t) CONSTFUNCTION;
template <typename T> inline T normalize_time(T t) { return T(TIME_SCALE) * t; }

static inline int tomilli(float x) CONSTFUNCTION;
inline int tomilli(float x) { return int(ceilf(1000.0f * normalize_time(x))); }

static inline int tomilli(double x) CONSTFUNCTION;
inline int tomilli(double x) { return int(ceil(1000.0 * normalize_time(x))); }
...
struct BuildSymbolMetadata : public std::vector<BuildSymbolFrameMetadata> {
    ...
    uint32_t getFileId(uint32_t build_framenum) const {
        if (symbol == NULL) {
            throw logic_error("Attempt to access BuildSymbolMetadata object "
                              "without prior setting of the symbol pointer.");
        }
        return symbol->getFrameIndex(build_framenum);
    }

    const BuildSymbolFrameMetadata &
    getFrameMetadata(uint32_t build_framenum) const {
...
struct AnimationFrameExporterState : public GenericState {
    // This is the same as the current key_id of the outer
    // AnimationExporterState.
    ...
    AnimationFrameExporterState(uint32_t _animation_frame_id, int max_z_index)
        : animation_frame_id(_animation_frame_id), object_ref_id(0),
          z_index(max_z_index) {
...
class AnimationSymbolFrameMetadata {
  ...
  public:
    ...
    AnimationSymbolFrameMetadata(const KAnim::Frame::Element &elem,
    ...
    void initialize(const KAnim::Frame::Element &elem, uint32_t _start_time) {
        if (M != NULL) {
            throw logic_error(("Reinitializing AnimationSymbolFrameMetadata "
                               "for frame element " +
                               elem.getName() + ".")
                                  .c_str());
        }
    ...
    const matrix_type &getMatrix() const {
        if (M == NULL) {
            throw logic_error("Attempt to fetch matrix from uninitialized "
                              "AnimationSymbolFrameMetadata.");
        }
        return *M;
    }

    uint32_t getBuildFrame() const { return build_frame; }
...
class AnimationSymbolMetadata : public deque<AnimationSymbolFrameMetadata> {
    ...
    void setDupedName() {
        DataFormatter fmt;
        timeline.attribute("name") =
            fmt("%s_%d", timeline.attribute("name").as_string(), dupeness);
    }

  public:
    AnimationSymbolMetadata(int _dupecnt = 0)
        : timeline(), id(0), dupeness(_dupecnt), is_dupe(_dupecnt > 0) {}

    /*
     * If no timeline node has been created yet, creates one and sets its
     * attributes.
     *
     * Also initializes corresponding element indexed by the elem build frame.
     */
    void initialize(xml_node animation, uint32_t &current_timeline_id,
                    uint32_t start_time, const KAnim::Frame::Element &elem) {
    ...
    xml_node getTimeline() const {
        if (!timeline) {
            throw logic_error("Attempt to fetch timeline node from "
                              "uninitialized AnimationSymbolMetadata object.");
        }
        return timeline;
    }
    ...
    uint32_t getTimelineId() const {
        if (!timeline) {
            throw logic_error("Attempt to fetch timeline id from uninitialized "
                              "AnimationSymbolMetadata object.");
        }
        return id;
    }

    int getKeyId() const { return int(size()) - 1; }
};

/*
 * Maps build symbol hashes to their metadata (as animation frame elements).
 *
 * Each value is a list, where a new element is added when a new timeline is
 * required (due to an object appearing multiple times, such as machine_leg for
 * the researchlab).
 */
class AnimationMetadata : public map<const KAnim::Frame::Element *,
                                     std::list<AnimationSymbolMetadata>,
                                     ptrLess<KAnim::Frame::Element> > {
    xml_node animation;

    // Counts how many times an element hash has been added.
    std::multiset<hash_t> elemhash_mset;

    // Maps an element hash to the first AnimationSymbolMetadata referring to
    // it.
  ...
  public:
    ...
    initializeChild(uint32_t &current_timeline_id, uint32_t start_time,
        ...
        for (symbolmeta_list::iterator it = L.begin(); it != L.end(); ++it) {
            ...
            if (animsymmeta.back().getStartTime() < start_time) {
                animsymmeta.initialize(animation, current_timeline_id,
                                       start_time, elem);
                return animsymmeta;
            }
        }
        ...
        if (dupeness == 0) {
            ...
            AnimationSymbolMetadata *first_animsymmeta =
                first_comers[elem.getHash()];
            if (first_animsymmeta == NULL) {
                throw std::logic_error(
                    "first_comers mapped to NULL, but elemhash_mset has a "
                    "count greater than 0.");
            }
            first_animsymmeta->setDupe();
        }
        animsymmeta.initialize(animation, current_timeline_id, start_time,
                               elem);
...
static void exportBuild(xml_node spriter_data, BuildMetadata &bmeta,
                        const KBuild &bild);

static void exportBuildSymbol(xml_node spriter_data, BuildExporterState &s,
                              BuildSymbolMetadata &symmeta,
                              const KBuild::Symbol &sym);

static void exportBuildSymbolFrame(xml_node folder, BuildSymbolExporterState &s,
                                   BuildSymbolFrameMetadata &framemeta,
                                   const KBuild::Symbol::Frame &frame);

static void exportAnimationBankCollection(xml_node spriter_data,
                                          const BuildMetadata &bmeta,
                                          const KAnimBankCollection &C);

static void exportAnimationBank(xml_node spriter_data,
                                AnimationBankCollectionExporterState &s,
                                const BuildMetadata &bmeta, const KAnimBank &B);

static void exportAnimation(xml_node entity, AnimationBankExporterState &s,
                            const BuildMetadata &bmeta, const KAnim &A);

static void exportAnimationFrame(xml_node mainline, AnimationExporterState &s,
                                 AnimationMetadata &animmeta,
                                 const BuildMetadata &bmeta,
                                 const KAnim::Frame &frame);

static void exportAnimationFrameElement(xml_node mainline_key,
                                        AnimationFrameExporterState &s,
                                        AnimationSymbolMetadata &animsymmeta,
                                        const BuildSymbolMetadata &symmeta,
                                        const KAnim::Frame::Element &elem);

static void
exportAnimationSymbolTimeline(const BuildSymbolMetadata &symmeta,
                              const AnimationSymbolMetadata &animsymmeta);

/***********************************************************************/

void Krane::exportToSCML(std::ostream &out, const KBuild &bild,
...
static void exportBuild(xml_node spriter_data, BuildMetadata &bmeta,
    ...
    for (symbolmap_iterator it = bild.symbols.begin(); it != bild.symbols.end();
         ++it) {
        hash_t h = it->first;
...
static void exportBuildSymbol(xml_node spriter_data, BuildExporterState &s,
    ...
    for (frame_iterator it = sym.frames.begin(); it != sym.frames.end(); ++it) {
        ...
        exportBuildSymbolFrame(folder, local_s, symmeta[local_s.file_id],
                               frame);
    }
...
static void exportBuildSymbolFrame(xml_node folder, BuildSymbolExporterState &s,
    ...
    // cout << "Exporting build symbol frame #" << file_id << ": " <<
    // frame.getUnixPath() << endl;
...
static void exportAnimationBankCollection(xml_node spriter_data,
    ...
    for (KAnimBankCollection::const_iterator bankit = C.begin();
         bankit != C.end(); ++bankit) {
...
static void exportAnimationBank(xml_node spriter_data,
    ...
    for (KAnimBank::const_iterator animit = B.begin(); animit != B.end();
         ++animit) {
...
static void exportAnimation(xml_node entity, AnimationBankExporterState &s,
    ...
    animation.append_attribute("name") =
        A.getFullName().c_str(); // BUILD_PLAYER ?
    ...
    for (KAnim::framelist_t::const_iterator frameit = A.frames.begin();
         frameit != A.frames.end(); ++frameit) {
    ...
    for (AnimationMetadata::const_iterator animsymlistit = animmeta.begin();
         animsymlistit != animmeta.end(); ++animsymlistit) {
        BuildMetadata::const_iterator symmeta_match =
            bmeta.find(animsymlistit->first->getHash());

        if (symmeta_match == bmeta.end()) {
            throw logic_error(
                "Program logic state invariant breached: hashes of keys of "
                "AnimationMetadata are not a subset of keys of BuildMetadata.");
        }

        const AnimationMetadata::list_type &animsymlist = animsymlistit->second;

        for (AnimationMetadata::list_type::const_iterator animsymit =
                 animsymlist.begin();
             animsymit != animsymlist.end(); ++animsymit) {
...
static void exportAnimationFrame(xml_node mainline, AnimationExporterState &s,
    ...
    for (KAnim::Frame::elementlist_t::const_iterator elemit =
             frame.elements.begin();
         elemit != frame.elements.end(); ++elemit) {
        const KAnim::Frame::Element &elem = *elemit;

        BuildMetadata::const_iterator symmeta_match =
            bmeta.find(elem.getHash());
        if (symmeta_match == bmeta.end()) {
            continue;
        }
        ...
        AnimationSymbolMetadata &animsymmeta =
            animmeta.initializeChild(s.timeline_id, start_time, elem);

        exportAnimationFrameElement(mainline_key, local_s, animsymmeta, symmeta,
                                    elem);
    }
}
...
static void exportAnimationFrameElement(xml_node mainline_key,
    ...
    const BuildSymbolFrameMetadata &bframemeta =
        symmeta.getFrameMetadata(build_frame);
    ...
    // Sanity checking.
    {
        ...
        if (sort_order < s.last_sort_order) {
            throw logic_error("Program logic state invariant breached: anim "
                              "sort order progression is not monotone.");
        }
        s.last_sort_order = sort_order;
    }

    // cout << "Exporting animation frame element " << elem.getName() << ",
    // build frame " << elem.getBuildFrame() << ", key id " <<
    // animsymmeta.getTimelineId() << endl;
    ...
    object_ref.append_attribute("file") = build_frame; // This is where
    deduplication would need to be careful.
    */
    object_ref.append_attribute("abs_x") = 0;
    object_ref.append_attribute("abs_y") = 0;
    object_ref.append_attribute("abs_pivot_x") =
        bframemeta.pivot_x; //*MODTOOLS_SCALE;
    object_ref.append_attribute("abs_pivot_y") =
        bframemeta.pivot_y;                       //*MODTOOLS_SCALE;
    ...
    object_ref.append_attribute("key") =
        animsymmeta
            .getKeyId(); // build_frame; // This changed for deduplication.
...
struct matrix_components {
    ...
    matrix_components() : scale_x(1), scale_y(1), angle(0), spin(1) {}
};

// sup entry-wise norm
template <typename MatrixTypeA, typename MatrixTypeB>
static typename MatrixTypeA::value_type matrix_dist(const MatrixTypeA &A,
                                                    const MatrixTypeB &B) {
...
/*
 * Decomposes (approximately if this is not possible exactly) a matrix into xy
 * scalings and a rotation angle in radians.
 *
...
static void decomposeMatrix(const MatrixType &M, matrix_components &ret,
                            matrix_components &last, bool &is_first) {
    ...
    if (fabs(ret.scale_x) < eps || fabs(ret.scale_y) < eps) {
        ...
        using namespace std;
        const float_type sin_approx =
            0.5 * (M[1][0] / ret.scale_y - M[0][1] / ret.scale_x);
        const float_type cos_approx =
            0.5 * (M[0][0] / ret.scale_x + M[1][1] / ret.scale_y);
...
exportAnimationSymbolTimeline(const BuildSymbolMetadata &symmeta,
    ...
    int key_id = 0;
    for (AnimationSymbolMetadata::const_iterator animsymframeiter =
             animsymmeta.begin();
         animsymframeiter != animsymmeta.end(); ++animsymframeiter) {
        const AnimationSymbolFrameMetadata &animsymframemeta =
            *animsymframeiter;

        xml_node timeline_key = timeline.append_child("key");

        timeline_key.append_attribute("id") =
            key_id++; // build_frame; // This changed for deduplication.
        timeline_key.append_attribute("time") =
            animsymframemeta
                .getStartTime(); // tomilli(frame_duration*bframemeta.duration);//tomilli(frame_duration*bframemeta.duration);
        ...
        if (options::check_animation_fidelity) {
            float_type md =
                matrix_dist(M, scaleMatrix(geo.scale_x, geo.scale_y) *
                                   rotationMatrix(geo.angle));
            if (md > MATRIX_EPS &&
                (symbol_badness == nil || symbol_badness.value() < md)) {
        ...
        object.append_attribute("file") = symmeta.getFileId(
            animsymframemeta
                .getBuildFrame()); // build_frame;//animsymframemeta.getBuildFrame();
    ...
    if (symbol_badness != nil) {
        cerr << "WARNING: the geometry of animation symbol \""
             << animsymmeta.getName() << "\" in animation \""
             << animsymmeta.getAnimationName()
             << "\" is not fully representable in Spriter. Expect lower "
                "animation fidelity. (max matrix distance: "
             << symbol_badness.value() << ")" << endl;
    }
}
