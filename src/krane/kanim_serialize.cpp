...
namespace Krane {
const uint32_t GenericKAnimFile::MAGIC_NUMBER =
    *reinterpret_cast<const uint32_t *>("ANIM");

const int32_t GenericKAnimFile::MIN_VERSION = 3;
const int32_t GenericKAnimFile::MAX_VERSION = 4;

bool GenericKAnimFile::shouldHaveHashTable() const { return version >= 4; }
  ...
  public:
    ...
    virtual std::string getBankName(hash_t h) const {
        return resolveHash(h,
                           "Incomplete anim hash table (missing bank name).");
    }
    virtual std::string getEventName(hash_t h) const {
        return resolveHash(h,
                           "Incomplete anim hash table (missing event name).");
    }
    virtual std::string getElementName(hash_t h) const {
        return resolveHash(
            h, "Incomplete anim hash table (missing element name).");
    }
    virtual std::string getLayerName(hash_t h) const {
        return resolveHash(h,
                           "Incomplete anim hash table (missing layer name).");
    }
};
...
class FallbackKAnimNamer : public KAnimNamer {
    ...
    std::string resolveHash(hash_t h, hashtable_t& cache, const char* prefix)
...
istream &GenericKAnimFile::load(istream &in, int verbosity) {
    ...
    if (!shouldHaveHashTable() || !in || in.peek() == EOF) {
        if (shouldHaveHashTable()) {
            std::cerr << "WARNING: Missing hash table at the end of the anim "
                         "file. Generating automatic names."
                      << std::endl;
        }
    ...
    if (in.peek() != EOF) {
        std::cerr << "Warning: There is leftover data in the input anim file."
                  << std::endl;
    }
...
istream &KAnim::loadPre(istream &in, int verbosity) {
    ...
    for (framelist_t::iterator it = frames.begin(); it != frames.end(); ++it) {
        if (verbosity >= 2) {
            framecounter++;
            cout << "\tLoading animation frame " << framecounter << "..."
                 << endl;
        }
        it->setParent(this);
...
istream &KAnim::loadPost(istream &in, const KAnimNamer &ht, int verbosity) {
    ...
    */
    ...
    for (framelist_t::iterator it = frames.begin(); it != frames.end(); ++it) {
        if (verbosity >= 2) {
            framecounter++;
            cout << "\tPost-processing animation frame " << framecounter
                 << "..." << endl;
        }
...
istream &KAnim::Frame::loadPre(istream &in, int verbosity) {
    ...
    if (events.size() < numevents) {
        throw(
            KToolsError("Invalid anim file (hash collision in event names)."));
    }
    ...
    for (elementlist_t::iterator it = elements.begin(); it != elements.end();
         ++it) {
        it->setParent(this);
        if (!it->loadPre(in, verbosity)) {
...
istream &KAnim::Frame::loadPost(istream &in, const KAnimNamer &ht,
                                int verbosity) {
    ...
    if(1) {
        ...
        for(elementlist_t::iterator it = elements.begin(); it != elements.end();
    ++it) { myset_t& myset = elem_layer_set[it->getHash()]; myset_t::value_type
    val = std::make_pair( it->layername_hash, it->z ); if(myset.count(val)) {
                std::cerr << ("Same element with duplicate layers: " +
    it->getName() + ", " + it->layername) << std::endl;
            }
            myset.insert(val);
        }
    }
...
istream &KAnim::Frame::Element::loadPre(istream &in, int verbosity) {
    ...
    if (verbosity >= 4) {
        cout << "\t\t\tLoading element 0x" << hex << hash << dec << "..."
             << endl;
    }
...
istream &KAnim::Frame::Element::loadPost(istream &in, const KAnimNamer &ht,
                                         int verbosity) {
