...
namespace Krane {
const uint32_t KBuildFile::MAGIC_NUMBER =
    *reinterpret_cast<const uint32_t *>("BILD");

const int32_t KBuildFile::MIN_VERSION = 5;
const int32_t KBuildFile::MAX_VERSION = 6;

bool KBuildFile::shouldHaveHashTable() const { return version >= 6; }
...
istream &KBuild::load(istream &in, int verbosity) {
    ...
    for (symbol_iter it = symbols.begin(); it != symbols.end(); ++it) {
        if (verbosity >= 4) {
            cout << "\tPost-processing symbol 0x" << hex << it->first << dec
                 << "..." << endl;
        }
    ...
    if (!parent->shouldHaveHashTable() || !in || in.peek() == EOF) {
        if (parent->shouldHaveHashTable()) {
            std::cerr << "WARNING: Missing hash table at the end of the build "
        ...
        for (uint32_t i = 0; i < hashcollection_sz; i++) {
            ...
            if (symb == NULL) {
                ...
                if (strhash(symb->name) != symb->hash) {
                    throw KToolsError("Hash for build symbol '" + symb->name +
                                      "' does not match its name.");
                }
            }

            if (verbosity >= 2) {
                cout << "\tGot 0x" << hex << h << dec << " => \"" << symb->name
                     << "\"" << endl;
            }
        }
        if (num_namedsymbols != numsymbols) {
            if (num_namedsymbols < numsymbols) {
                throw(KToolsError(
                    "Incomplete build hash table (missing symbol name)."));
            } else {
    ...
    if (in.peek() != EOF) {
        std::cerr << "Warning: There is leftover data in the input build file."
                  << std::endl;
    }
...
istream &KBuild::Symbol::loadPre(istream &in, int verbosity) {
    ...
    for (uint32_t i = 0; i < numframes; i++) {
        if (verbosity >= 3) {
            cout << "\t\tReading symbol frame at position #" << (i + 1) << "..."
                 << endl;
        }
        Frame &f = frames[i];
        f.setParent(this);
...
istream &KBuild::Symbol::Frame::loadPre(istream &in, int verbosity) {
    ...
    if (alphacount % 6 != 0) {
        throw(KToolsError("Corrupted build file (frame VB count should be a "
                          "multiple of 6)."));
    }
...
istream &KBuild::Symbol::loadPost(istream &in, int verbosity) {
    ...
    for (size_t i = 0; i < numframes; i++) {
        if (verbosity >= 3) {
            cout << "\t\tPost-processing symbol frame at position #" << (i + 1)
                 << "..." << endl;
        }
        if (!frames[i].loadPost(in, verbosity)) {
            throw(
                KToolsError("Failed to post-process animation symbol frame."));
        }
    }
    ...
    for (uint32_t i = 0; i < numtrigs; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (uvwdepth == nil) {
                ...
                throw KToolsError(
                    "Inconsistent uvw depth in build symbol frame.");
            }

            if (!in) {
                throw(KToolsError(
                    "Corrupt build file (failed to read VB vertex)."));
            }
        }
