...
#else
#define BOOST_COMP_NVCC_DETECTION                                              \
    BOOST_VERSION_NUMBER(__CUDACC_VER_MAJOR__, __CUDACC_VER_MINOR__,           \
...
This behavior is similar to what boost config is doing in
`select_compiler_config.hpp`. There the NVCC detection is not handled as a real
compiler (part of the #if-#elif) but as additional option before the real
compiler.
*/
